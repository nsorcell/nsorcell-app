...
import { transformFetchStatsResult } from "utils/store"

const fetchStatsEpic: Epic<AnyAction, AnyAction, State> = (action$, state$) =>
  action$.pipe(
    ...
    switchMap(() => {
      ...
      return from(
        ...
        map(pipe(transformFetchStatsResult, fetchStatsReceived)),
